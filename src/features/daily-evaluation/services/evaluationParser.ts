...
export interface ParseProgressCallbacks {
  setCalcStatus: (status: string) => void;
  setCalcProgress: (pct: number) => void;
}

// Full daily-evaluation parsing pipeline, moved verbatim out of
// DailyEvaluationGraph.tsx. Builds 86,400-slot (1 Hz) aligned series for all
// signals x 3 plants, interpolates/forward-fills gaps, computes cycle numbers
// (ESS cycle sheets when present, power-curve fallback otherwise), SOC stats
// and inter-plant deviation timings. Throws on invalid input; the caller owns
// UI state (progress, error banner) and persistence of the returned dataset.
export const parseEvaluationFiles = async (
  files: EvalFileEntry[],
  project: string,
  { setCalcStatus, setCalcProgress }: ParseProgressCallbacks
): Promise<EvalData> => {
