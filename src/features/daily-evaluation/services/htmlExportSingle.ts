...
// Standalone portable-HTML export of the CURRENT figure/plant. Body moved
// verbatim from DailyEvaluationGraph.tsx (handleExportHtml).
//
// NOTE: this template and its "all graphs" sibling (htmlExportAll.ts) have
// drifted apart in ~150 lines beyond the intended metric-switcher difference
// (rounding, showlegend rules, fig5 info-box implementation, theme reset).
// See docs/template-drift.diff. Unifying them WOULD CHANGE exported output —
// deliberately deferred until the desired behavior per divergence is decided.
export const exportSingleGraphHtml = async ({ evalData, project, showNccPCommand, graphConfig, activeMetric, selectedPlant, pinnedPoints }: HtmlExportContext): Promise<void> => {
    if (!evalData) return;
        ...
        for (let i = 0; i < imageUrls.length; i++) {
          ...
          if (activeMetric === 'fig5' && typeof evalDataRaw !== 'undefined' && evalDataRaw.dailyCycle && evalDataRaw.totalCycle) {
            ...
            if (i === 0 && evalDataRaw.hasCycleData) {
              ...
              if (hasPlant2) lines.push('Cycle_Plant 02 = ' + evalDataRaw.dailyCycle.plant2.toFixed(3) + ' -> ' + getStatus(evalDataRaw.dailyCycle.plant2));
