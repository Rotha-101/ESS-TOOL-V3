import type { HtmlExportContext } from './htmlExportSingle';

// Standalone portable-HTML export with ALL figures + a client-side metric
// switcher. Body moved verbatim from DailyEvaluationGraph.tsx
// (handleExportAllHtml). See the drift note in htmlExportSingle.ts.
export const exportAllGraphsHtml = async ({ evalData, project, showNccPCommand, graphConfig, activeMetric, selectedPlant, pinnedPoints }: HtmlExportContext): Promise<void> => {
    if (!evalData) return;
